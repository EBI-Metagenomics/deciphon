...
int product_thread_setup(struct product_thread *x, char const *abc, char const *dir)
{
  ...
  if ((rc = format(name, n, "%s/.products.%03d.tsv", dir, x->id))) return error(rc);
  if ((rc = fs_touch(x->filename)))                                return error(rc);
  product_line_init(&x->line);
...
int product_thread_add_hmmer(struct product_thread *x, struct h3r const *result)
{
  ...
  if ((rc = format(file, FS_PATH_MAX, "%s/hmmer/%ld", dir, seq)))                              return error(rc);
  if ((rc = fs_mkdir(file, true)))                                                             return error(rc);
  if ((rc = format(file, FS_PATH_MAX, "%s/hmmer/%ld/%d", dir, seq, win)))                      return error(rc);
  if ((rc = fs_mkdir(file, true)))                                                             return error(rc);
  if ((rc = format(file, FS_PATH_MAX, "%s/hmmer/%ld/%d/%d", dir, seq, win, hit)))              return error(rc);
  if ((rc = fs_mkdir(file, true)))                                                             return error(rc);
  if ((rc = format(file, FS_PATH_MAX, "%s/hmmer/%ld/%d/%d/%s.h3r", dir, seq, win, hit, prot))) return error(rc);
...
static int write_match(FILE *fp, struct match const *m)
{
  ...
  *ptr = ',';
  ...
  if (!match_state_is_mute(m))
  {
    ...
    if ((rc = match_codon(m, &codon))) return error(rc);
  ...
  *ptr++ = ',';
  ...
  {
    char amino = 0;
    if ((rc = match_amino(m, &amino))) return error(rc);
    *ptr++ = amino;
  }
  *ptr = '\0';

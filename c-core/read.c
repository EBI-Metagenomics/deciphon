...
{
  int rc = 0;
  unsigned char *buf = lio_read(x);
  if ((rc = lio_rerror(x)))                      return error_system(DCP_EFREAD, lio_syserror(rc));
...
int read_f32array(struct lio_reader *x, uint32_t size, float *array)
{
  ...
  if (lio_readb(x, n, (unsigned char *)array)) return error_system(DCP_EFREAD, lio_syserror(rc));
  return 0;
}

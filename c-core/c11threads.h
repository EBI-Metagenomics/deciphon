...
/* If you wish to use this with pthread-win32 (i.e. use the POSIX threads
 * wrapper instead of the native win32 API implementation of C11 threads), then
 * just define C11THREADS_PTHREAD_WIN32 before including this header file.
 */
...
static C11THREADS_INLINE int thrd_create(thrd_t *thr, thrd_start_t func,
                                         void *arg)
{
...
static C11THREADS_INLINE thrd_t thrd_current(void) { return pthread_self(); }
...
static C11THREADS_INLINE int thrd_sleep(const struct timespec *ts_in,
...
static C11THREADS_INLINE void thrd_yield(void) { sched_yield(); }
...
static C11THREADS_INLINE int mtx_timedlock(mtx_t *mtx,
...
static C11THREADS_INLINE int cnd_timedwait(cnd_t *cond, mtx_t *mtx,
                                           const struct timespec *ts)
{
  int res;
...
static C11THREADS_INLINE void tss_delete(tss_t key) { pthread_key_delete(key); }
...
/* C11 threads implementation using native Win32 API calls (see
 * c11threads_win32.c) */
...
/* Win32: Threads not created with thrd_create() need to call this to clean up
 * TSS. */
...
/* Win32: Register current Win32 thread in c11threads to allow for proper
 * thrd_join(). */
int c11threads_win32_thrd_self_register(void);
/* Win32: Register Win32 thread by ID in c11threads to allow for proper
 * thrd_join(). */
...
int _c11threads_win32_thrd_sleep32(
    const struct _c11threads_win32_timespec32_t *ts_in,
    struct _c11threads_win32_timespec32_t *rem_out);
int _c11threads_win32_thrd_sleep64(
    const struct _c11threads_win32_timespec64_t *ts_in,
                                        ...
                                        struct timespec *rem_out)
{
  ...
  {
    return _c11threads_win32_thrd_sleep32(
        (const struct _c11threads_win32_timespec32_t *)ts_in,
        (struct _c11threads_win32_timespec32_t *)rem_out);
  }
  else
  {
    return _c11threads_win32_thrd_sleep64(
        (const struct _c11threads_win32_timespec64_t *)ts_in,
        (struct _c11threads_win32_timespec64_t *)rem_out);
  }
}
...
int _c11threads_win32_mtx_timedlock32(
    mtx_t *mtx, const struct _c11threads_win32_timespec32_t *ts);
int _c11threads_win32_mtx_timedlock64(
    mtx_t *mtx, const struct _c11threads_win32_timespec64_t *ts);
static C11THREADS_INLINE int mtx_timedlock(mtx_t *mtx,
                                           const struct timespec *ts)
{
  if (sizeof(ts->tv_sec) == 4)
  {
    return _c11threads_win32_mtx_timedlock32(
        mtx, (const struct _c11threads_win32_timespec32_t *)ts);
  }
  else
  {
    return _c11threads_win32_mtx_timedlock64(
        mtx, (const struct _c11threads_win32_timespec64_t *)ts);
  }
}

/* ---- condition variables ---- */

int _c11threads_win32_cnd_timedwait32(
    cnd_t *cond, mtx_t *mtx, const struct _c11threads_win32_timespec32_t *ts);
int _c11threads_win32_cnd_timedwait64(
    cnd_t *cond, mtx_t *mtx, const struct _c11threads_win32_timespec64_t *ts);
static C11THREADS_INLINE int cnd_timedwait(cnd_t *cond, mtx_t *mtx,
                                           const struct timespec *ts)
{
  if (sizeof(ts->tv_sec) == 4)
  {
    return _c11threads_win32_cnd_timedwait32(
        cond, mtx, (const struct _c11threads_win32_timespec32_t *)ts);
  }
  else
  {
    return _c11threads_win32_cnd_timedwait64(
        cond, mtx, (const struct _c11threads_win32_timespec64_t *)ts);
  }
}
...
#ifdef C11THREADS_NO_TIMESPEC_GET
int _c11threads_win32_timespec32_get(struct _c11threads_win32_timespec32_t *ts,
                                     int base);
int _c11threads_win32_timespec64_get(struct _c11threads_win32_timespec64_t *ts,
                                     int base);
static C11THREADS_INLINE int timespec_get(struct timespec *ts, int base)
{
  if (sizeof(ts->tv_sec) == 4)
  {
    return _c11threads_win32_timespec32_get(
        (struct _c11threads_win32_timespec32_t *)ts, base);
  }
  else
  {
    return _c11threads_win32_timespec64_get(
        (struct _c11threads_win32_timespec64_t *)ts, base);
  }
}
#endif

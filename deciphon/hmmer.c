               ...
               int deadline)
{
  ...
  int rc = h3client_stream_put(x->stream, cmd, name, seq,
                               h3client_deadline(deadline));
  ouch("(%p) seq: %s", x->stream, seq);
  if (rc)
  {
    ouch("(%p) h3client_stream_put error: %s", x->stream,
         h3client_strerror(rc));
    return rc;
  }
  return 0;
}
...
static int pop(struct hmmer *x)
{
  ...
  int rc = h3client_stream_pop(x->stream, x->result.handle);
  if (elapsed_stop(&x->elapsed)) return DCP_EELAPSED;
  long mseconds = elapsed_milliseconds(&x->elapsed);
  if (rc)
  {
    ouch("(%p) h3client_stream_pop error [%ld msecs]: %s", x->stream, mseconds,
         h3client_strerror(rc));
    return rc;
  }
  else
...
int hmmer_warmup(struct hmmer *x)
{
  ...
  {
    ouch("hmmer_warmup put error: %s", h3client_strerror(rc));
    return rc;
  }
  if ((rc = pop(x)))
  {
    ouch("hmmer_warmup pop error: %s", h3client_strerror(rc));
    return rc;
  }
  return rc;
}
...
int hmmer_get(struct hmmer *x, int hmmidx, char const *name, char const *seq)
{
  for (int i = 0; i < NUM_TRIALS; ++i)
  {
    ...
    if ((rc = pop(x)) == H3CLIENT_ETIMEDOUT) continue;
